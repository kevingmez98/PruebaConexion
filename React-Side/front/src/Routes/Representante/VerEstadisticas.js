...
//Funcion para ver una lista de un solo representante
function VerEstadisticas() {
    ...
    var peticionEstadisticas = () => {
        return new Promise((resolve, reject) => {
            setMessage("");
            Axios.post('http://localhost:8080/representante/verEstadisticas', { "Serial": window.sessionStorage.getItem("Serial") })
                .then((response) => {
    ...
    return (
        <React.Fragment>
            <CardComponent titulo={"Ver representantes a cargo"}>
                <div className="p-3 mb-2 bg-info text-white">Estadisticas</div>
                {/*Mensaje de error */}
                {ErroMessage && (
